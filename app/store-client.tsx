...
function belongsToCategory(product: Product, categoryId: string): boolean {
  ...
  return product.filters.includes(categoryId);
}
...
export function StoreClient({ categories, products }: StoreClientProps) {
  ...
  const productCounts = useMemo(() => {
    ...
    for (const cat of categories) {
      if (cat.id !== "todos") {
        counts[cat.id] = products.filter((p) => belongsToCategory(p, cat.id)).length;
      }
    }
    return counts;
  }, [categories, products]);
  ...
  const filtered = useMemo(
    () =>
      selectedCategory === "todos"
        ? products
        : products.filter((p) => belongsToCategory(p, selectedCategory)),
    [products, selectedCategory]
  );

  // Agrupa variantes com mesmo nome (ex: Marau Areia + Marau Bronze → 1 card)
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: "var(--color-store-beige-dark)" }}>
                {grouped.length} modelo{grouped.length !== 1 ? "s" : ""}{" "}
                encontrado{grouped.length !== 1 ? "s" : ""}
              </p>
            </div>
          </div>
          ...
          {grouped.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
              {grouped.map((group) => (
                <ProductCard key={`${group[0].category}::${group[0].name}`} products={group} />
              ))}
            </div>
          ) : (
            <div
